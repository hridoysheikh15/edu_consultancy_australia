(function ($) {
    ...
    if ($cursor.length) { // Check if .cursor exists
        $(document).on('mousemove', function (e) {
            let cursorSize = 15;
            let x = Math.max(cursorSize, Math.min(e.pageX - cursorSize, $(window).width() - cursorSize * 2))
            let y = Math.max(cursorSize, Math.min(e.pageY - cursorSize, $(window).height() - cursorSize * 2))
            $cursor.css({
                top: e.pageY - 15 + 'px',
                left: x + 'px'
            });
